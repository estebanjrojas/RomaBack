...
exports.insertPersonaReturnId = function (req, res) {
    ...
    querySrv.getQueryResults(qPersonas.insertReturingId, [nro_doc, tipo_doc, apellido, nombre, telefono, celular, email, fecha_nac, sexo, tipo_persona, usuario, estado_civil, fecha_cese, usuario_carga, fecha_carga, telefono_caracteristica, celular_caracteristica, domicilios_id])
    .then(response => res.send({ "mensaje": "La persona se cargo exitosamente", "id":response.value[0].id}))
...
exports.getPersonaPorNroDoc = function (req, res) {
    querySrv.getQueryResults(qPersonas.getPersonaPorNroDoc, [req.params.tipo_doc, req.params.nro_doc])
