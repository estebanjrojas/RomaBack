...
exports.getEmpleadosTodos = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getEmpleadosTodos, [])
...
exports.getEmpleadosBusqueda = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getEmpleadosBusqueda, [req.params.texto_busqueda])
    .then(response => res.send(JSON.stringify(response.value)))
    .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
}

exports.getEmpleadoPorNroDoc = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getEmpleadoPorNroDoc, [req.params.tipo_doc, req.params.nro_doc])
    .then(response => res.send(JSON.stringify(response.value)))
    .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
}

exports.getDatosEmpleadoPorId = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getDatosEmpleadoPorId, [req.params.id])
    .then(response => res.send(JSON.stringify(response.value)))
    .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
}

exports.getEmpleadosSinUsuario = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getEmpleadosSinUsuario, [])
    .then(response => res.send(JSON.stringify(response.value)))
    .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
}
...
exports.getCantidadPaginasEmpleados = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getCantidadPaginasEmpleados, [])
...
exports.getEmpleados = function (req, res) {
    querySrv.getQueryResults(qEmpleados.getEmpleados, [req.params.paginaActual, req.params.cantidadPaginas])
    .then(response => res.send(JSON.stringify(response.value)))
    .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
}
...
exports.insertEmpleadoReturnId = function (req, res) {
    ...
    const fecha_ingreso = (req.body.fecha_ingreso != undefined) ? `'` + req.body.fecha_ingreso + `'` : `now()::date`;
    const descripcion = (req.body.descripcion != undefined) ? `'` + req.body.descripcion + `'` : `null`;
    const empresas_id = (req.body.empresas_id != undefined) ? req.body.empresas_id : `null`;
    const oficina = (req.body.oficina != undefined) ? req.body.oficina : `null`;
    
    querySrv.getQueryResults(qEmpleados.insertEmpleadoReturnId, [personas_id, legajo, fecha_ingreso, descripcion, empresas_id, oficina])
    .then(response => res.send({ "mensaje": "El empleado se cargo exitosamente", "id": response.value[0].id }))
    .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
}
...
exports.guardarEmpleadoPersonaDomicilio = function (req, res) {
    ...
    let empleados_id = (req.body.empleados_id != undefined || req.body.empleados_id != '') ? req.body.empleados_id : null;
    ...
    .then(domiciliosResp => {
        ...
        .then(personaResp => {
            ...
            querySrv.getQueryResults(queryEmpleado.text, queryEmpleado.values)
            .then(empleadoResp => res.send({ "mensaje": "El empleado se cargo exitosamente", "id": empleadoResp.value[0].id }))
            .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido Error ${err}` })));
        })
        .catch(err => res.status(400).send(JSON.stringify({"mensaje": `Ha ocurrido error al cargar la persona ${err}` })));
